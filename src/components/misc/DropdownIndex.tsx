...
const DropdownIndex = ({title , options, onChange} : any) => {
    let initialTitle =  title === 'Exchange' ? 'Exchange' : title === 'Granularity' ? '1D' : 'Notional';
                ...
                <AnimatePresence>
                    {isOpen && (
                        <motion.div 
                            ...
                            id="dropdownDelay" 
                            className={  `block z-10 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-900 absolute max-h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-200 dark:scrollbar-thumb-zinc-800 scrollbar-track-white dark:scrollbar-track-zinc-600`}>
